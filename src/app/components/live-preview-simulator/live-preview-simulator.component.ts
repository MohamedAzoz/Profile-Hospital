...
@Component({
  ...
  template: `
    <section id="demo" *transloco="let t" class="py-24 relative overflow-hidden bg-linear-to-b from-blue-900/10 via-transparent to-transparent">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        <!-- Header -->
        <div class="text-center max-w-3xl mx-auto space-y-4 mb-16">
          <span class="px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider bg-purple-100 dark:bg-purple-900/50 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-700">
            {{ t('preview.tag') }}
          </span>
        ...
        <div class="glass-card rounded-3xl p-6 sm:p-10 border border-raw-border shadow-2xl space-y-8">
          ...
          <div class="flex items-center justify-between pb-6 border-b border-raw-border">
            <div class="flex items-center gap-3">
              ...
              <span class="text-xs sm:text-sm font-bold text-raw-text tracking-wide">
                PulseCare Hospital Live Console — [{{ langService.currentLang().toUpperCase() }} | {{ themeService.theme().toUpperCase() }}]
              </span>
            </div>

            <div class="flex items-center gap-2 text-xs font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 px-3 py-1 rounded-full border border-emerald-500/20">
              <span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
              <span>Signals Stream Active</span>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="glass p-6 rounded-2xl border border-raw-border space-y-6 flex flex-col justify-between">
              <div>
                <div class="flex items-center justify-between">
                  ...
                  <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-amber-500/20 text-amber-600 dark:text-amber-300">
                    Live Queue
                  </span>
                </div>

                <!-- Current Calling Ticket Box -->
                <div class="mt-6 p-6 rounded-2xl bg-linear-to-tr from-blue-600 to-indigo-600 text-white text-center shadow-lg relative overflow-hidden">
                ...
                <div class="mt-4 p-4 rounded-xl bg-raw-surface border border-raw-border flex items-center justify-between">
                  <div>
                    <span class="text-xs text-raw-text-muted block">{{ t('preview.nextPatient') }}</span>
                    @if (demoService.nextPatientInQueue(); as nextP) {
                      <span class="text-sm font-bold text-raw-text">
                        #{{ nextP.ticketNumber }} — {{ langService.isRtl() ? nextP.nameAr : nextP.nameEn }}
                      </span>
                    } @else {
                      <span class="text-xs text-raw-text-muted">No waiting patients</span>
                    }
                  </div>
                  <span class="text-xs font-semibold px-2 py-1 rounded bg-blue-500/10 text-blue-600">
                    Waiting
                  </span>
                </div>
              </div>
            ...
            <!-- Panel 2: Live Vitals Monitor (Zoneless Stream) -->
            <div class="glass p-6 rounded-2xl border border-raw-border space-y-6 flex flex-col justify-between">
              <div>
                <div class="flex items-center justify-between mb-4">
                  ...
                  <span class="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-500/20 text-emerald-600 dark:text-emerald-300 animate-pulse">
                    ECG Live
                  </span>
                </div>

                <!-- Heart Rate Counter Box -->
                <div class="p-4 rounded-2xl bg-raw-surface border border-raw-border flex items-center justify-between mb-4 shadow-sm">
                  <div class="flex items-center gap-3">
                    <div class="w-12 h-12 rounded-xl bg-rose-500/10 text-rose-500 flex items-center justify-center text-2xl animate-bounce">
                      ❤️
                    </div>
                    <div>
                      <span class="text-xs text-raw-text-muted block">{{ t('preview.heartRate') }}</span>
                ...
                <div class="grid grid-cols-2 gap-3 mb-4">
                  <div class="p-4 rounded-2xl bg-raw-surface border border-raw-border">
                    <span class="text-xs text-raw-text-muted block">{{ t('preview.bloodPressure') }}</span>
                    <span class="text-xl font-extrabold text-raw-text">
                      {{ demoService.bloodPressureSys() }}/{{ demoService.bloodPressureDia() }}
                    </span>
                    <span class="text-[10px] text-raw-text-muted block mt-0.5">{{ t('preview.sysDia') }}</span>
                  </div>
                  ...
                  <div class="p-4 rounded-2xl bg-raw-surface border border-raw-border">
                    ...
                    <span class="text-[10px] text-raw-text-muted block mt-0.5">{{ t('preview.oxygenVal') }}</span>
                  </div>
                </div>

                <!-- Animated ECG Wave Canvas / Simulator Bar -->
                <div class="h-10 w-full rounded-xl bg-slate-900/90 border border-slate-700/60 flex items-center justify-between px-4 overflow-hidden">
            ...
            <div class="glass p-6 rounded-2xl border border-raw-border space-y-6 flex flex-col justify-between">
              <div>
                <div class="flex items-center justify-between mb-4">
                  ...
                  <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-purple-500/20 text-purple-600 dark:text-purple-300">
                    Rx Engine
                  </span>
                </div>
                ...
                <div class="space-y-2 max-h-44 overflow-y-auto pr-1">
                  ...
                  @if (demoService.prescriptions().length === 0) {
                    ...
                    @for (item of demoService.prescriptions(); track item.id) {
                      <div class="p-2.5 rounded-xl bg-raw-surface border border-raw-border flex items-center justify-between text-xs">
                        <div>
              ...
              <div class="space-y-2">
                @if (demoService.isPrescriptionSynced()) {
                  <div class="p-3 rounded-xl bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 text-xs font-bold text-center border border-emerald-500/30 animate-in fade-in">
